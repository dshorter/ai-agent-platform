...
**Correction 2026-09-01 — "fully stopped" and "zero publishing" above are wrong, and the error is instructive.** `agent-self-report-unreliable` released **2026-08-15** (`uzelhub-web` 553b06f), six days *before* this entry was written. The drip was never fully stopped; it was running at one note in the 34 days, against a 1–2/week contract. The stall is real — that is roughly a tenth of the target rate — but the entry reached for the dramatic number instead of checking `notes.json`, and a ledger is exactly the wrong place for that: it is the Director's memory across runs, so a wrong figure here is re-read as established fact rather than re-derived. **Standing lesson: a claim about what a stream did is a claim about a file, and the file was one `git log` away.** The original wording stays visible above per the correction convention.
